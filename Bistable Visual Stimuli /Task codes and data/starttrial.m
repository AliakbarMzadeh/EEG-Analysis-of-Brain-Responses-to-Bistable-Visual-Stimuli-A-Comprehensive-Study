function [jahat , treact , tshow] = starttrial(moviename, hdr, backgroundMaskOut, tolerance, pixelFormat, maxThreads)
...
try
    ...
    if isempty(strfind(moviename, 'http')) %#ok<STREMP>
       ...
       %     moviefiles(1).name = [ PsychtoolboxRoot 'PsychDemos/MovieDemos/DualDiscs.mov' ];
            moviefiles(1).name = [ PsychtoolboxRoot 'bp.mov' ];
        else
    ...
    while (abortit<2)
        ...
        % end of trial 90 seconds
            
        if(GetSecs() - tshow0 >= 90.047000)
           break;
        end
        ...
        while 1
            ...
            if (keyIsDown==1 && keyCode(rikey))
                timee = GetSecs();
                tdo = [ tdo , timee];
                treact = [treact , (timee - tshow)];
                jahat = [jahat , 1];
                tflag = GetSecs();
            end
            
            if (keyIsDown==1 && keyCode(lekey))
                timee = GetSecs();
                tdo = [ tdo , timee];
                treact = [treact , (timee - tshow)];
                jahat = [jahat , -1];
